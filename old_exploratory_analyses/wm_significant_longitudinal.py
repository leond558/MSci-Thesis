...
if __name__ == '__main__':
    ...
    regions_of_interest_dictionary = {
        'Anterior midbody/ Primary Motor ':[8],
        'Optic radiation Right': [30],
        'Superior longitudinal fascicle I Right': [36],
        'Striato-occipital Right': [71],
        'Inferior longitudinal fascicle Right': [27]

    }

    if existing_data:
        # Load pre-processed data from a pickle file
        with open('../pickles/md_wm_significant_longitudinal.pkl', 'rb') as f:
        ...
        malpem_dti = xlsx['singleshell_tractseg_FA_MD']
        ...
        with open('../pickles/md_wm_significant_longitudinal.pkl', 'wb') as f:
            pkl.dump(
    ...
    # Add a title
    ax.set_title(
        "Mean Diffusivity in White Matter Regions",
    ...
    plt.tight_layout()
    plt.savefig('plots/wm_significant.png')
    ...
    stats_df['p-value for acute to longitudinal'] = stats.wilcoxon(after_md_patients, before_md_patients)[1]
    stats_df['p-value for longitudinal to controls'] = stats.ranksums(after_md_patients, md_controls)[1]
    ...
    plt.savefig('plots/significant_wm_table.png')
